...
We summarized the distribution of TCSA values for each site to examine spatial patterns in TCSA values. We then explore possible environmental impact by connecting stemmed points on the map with eco-regional zones. We used QGIS 3.22.12 'Białowieża' (https://qgis.org/en/site/index.html) to draw a map based on the ecological study [CITE RESEARCH OF FINAL CHOICE] and used longitudes and latitudes of assemblages to locate the individual artifacts. Among `r n_sites` artifacts, we sat sites that contains fewer than 5 stemmed points aside and named them as "Other".
...
for_plot <- ggplot(points_in_veg_zones_forest_box) +
  aes(Zone,
      min_tcsa) +
  geom_boxplot() +
  geom_quasirandom() 
...
plot_grid(eco_plot, for_plot, eco_box, for_plot, labels = c('A', 'C', 'B', 'D'))
```
