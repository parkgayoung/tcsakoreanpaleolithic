...
We summarized the distribution of TCSA values for each assemblage to examine spatial patterns in TCSA values. We then explore possible environmental impact by connecting stemmed points on the map with eco-regional zones. We used QGIS 3.22.12 'Białowieża' (https://qgis.org/en/site/index.html) to draw a map based on the previous ecological studies [@yim1975distribution; @yi2010vegetation; @lee2008relationship] and located the individual artifacts using longitudes and latitudes of assemblages. Among `r n_sites` artifacts, we sat sites that contains fewer than 5 stemmed points aside and named them as "Other".
...
eco_plot <- ggplot() +
  geom_sf(data = veg_all, aes(fill = "grey")) +
   geom_sf_text(data = veg_all, aes(label = ecoregion), size =3.5)+
...
for_plot <- ggplot() +
  ...
  geom_sf_text(data = veg_forest, aes(label = Zone), size =3.5)+
